...
public class Word implements Serializable {
    ...
    private final static int BOXSIZE = 25;
    ...
    public Word(String word,String clue,int x,int y,boolean direction,int number){
        ...
        try {
            if (direction) {                                                                                            // check direction for algorithm ( horizontal )
                numPoint = new Point((x-BOXSIZE),y);                                                                    // number position just before the word
                IntStream.range(0, wordLen).forEach(i -> {                                                              // for each letter in word
                    charPositions[i] = new Point((x + (i * BOXSIZE)), y);                                               // assign position of letter
                });
            } else {                                                                                                    // check direction for algorithm ( vertical )
                numPoint = new Point(x,(y-BOXSIZE));                                                                    // number position just before the word
                IntStream.range(0, wordLen).forEach(i -> {                                                              // for each letter in word
                    charPositions[i] = new Point(x, (y + (i * BOXSIZE)));                                               // assign position of letter
                });
            }
        ...
        for (int i = 0; i < wordLen; i++) {
            spotConnected[i] = false;                                                                                   // set all connected values to false
            available[i] = true;                                                                                        // set all available values to false
        }
    } // end constructor
    ...
    public void renderClue(){
        Point drawPoint = new Point(820,(20*number));
